...
///
/// Rosenthal gives the list because the mistake is common: these sound like an
/// aside and are ordinary members of the sentence.
pub const NEVER: &[&str] = &[
    "авось",
    "буквально",
    "будто",
...
/// The words that are parenthetical or not by what they do in the sentence.
///
/// `однако` opens a clause as a conjunction and stands inside one as an aside;
/// `правда` is a noun and a concession both. A gate that meets one of these
/// reads the sentence and does not read a list.
...
/// belongs to [`EITHER`] or because it is no parenthetical at all.
///
...
pub fn needs_commas(written: &str) -> Option<bool> {
    ...
    if ALWAYS.contains(&held.as_str()) {
        return Some(true);
    }
    if NEVER.contains(&held.as_str()) {
        return Some(false);
    }
...
pub fn is_either(written: &str) -> bool {
    EITHER.contains(&written.to_lowercase().as_str())
}
...
mod tests {
    ...
    fn no_list_holds_a_word_twice_and_no_word_is_on_two_lists() {
        let mut every: Vec<&str> = [ALWAYS, NEVER, EITHER].concat();
