...
/// The particles.
///
/// Holds every particle, with the two subsets a checker asks about most:
/// the ones that build a form and the ones that deny, and the leaning
/// ones that cannot open a phrase.
///
/// # Examples
///
/// ```
/// use rusem::grammar::closed::particle::Particle;
///
/// assert!(Particle::PARTICLES.contains(&"же"));
/// assert!(Particle::DENYING.contains(&"не"));
/// ```
...
impl Particle {
    ...
    ///
    /// `бы` makes the subjunctive out of the past, `пусть` and `давай` make
    /// an imperative out of the present. They are part of the verb form and
    /// not words standing beside it, which is why a gate counting words in a
    /// predicate must not count them.
    pub const FORMATIVE: &[&str] = &["б", "бы", "давай", "давайте", "пускай", "пусть"];

    /// The particles that deny.
    ///
    /// `не` denies what follows it; `ни` denies it again and is what a rule
    /// on double negation counts alongside the first.
...
pub fn is_particle(written: &str) -> bool {
    Particle::PARTICLES.contains(&written.to_lowercase().as_str())
}
...
mod tests {
    ...
    fn every_denying_particle_and_every_enclitic_is_a_particle() {
        for held in Particle::DENYING.iter().chain(Particle::ENCLITIC) {
    ...
    fn a_formative_particle_is_a_particle() {
        for held in Particle::FORMATIVE {
            assert!(is_particle(held), "{held} is no particle");
        }
    }
    ...
    fn nothing_both_denies_and_leans() {
        assert!(!Particle::DENYING.iter().any(|held| leans(held)));
    }
}
