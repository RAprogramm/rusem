...
use super::{adjective::parted, attributive, spelling};
...
pub fn cells(written: &str, dictionary: &str, animacy: Animacy) -> Vec<Cell> {
    ...
    for gender in Gender::STATED {
        for case in Case::STATED {
            let table = attributive::table(shape, gender, Number::Singular);
            if writes(&base, table.of(case, animacy), written) {
                found.push(Cell {
                    case,
    ...
    let table = attributive::table(shape, Gender::Masculine, Number::Plural);
    for case in Case::STATED {
        if writes(&base, table.of(case, animacy), written) {
            found.push(Cell {
                case,
...
fn writes(stem: &str, ending: &str, written: &str) -> bool {
    [true, false]
        .into_iter()
        .any(|stressed| joins(stem, &spelling::fitted(stem, ending, stressed), written))
}
